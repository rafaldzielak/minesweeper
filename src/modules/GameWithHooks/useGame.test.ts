...
describe("useGame test cases", () => {
  ...
  describe("OnClick with OnChangeGameLevel and OnReset", () => {
    it("Check click to the cell when the level is changed", () => {
      ...
      expect(playerField).toHaveLength(9);
      act(() => onChangeLevel(intermediate));
      const { playerField: intermediatePlayerField, onClick: onClickIntermediate } = result.current;
      ...
      act(() => onChangeLevel(expert));
      const { playerField: expertPlayerField, onClick: onClickExpert } = result.current;
  ...
  describe("Game over behavior", () => {
    it("Player loose the game", () => {
      ...
      act(() => onClick([0, 7]));
      const { isWin, isGameOver, playerField: newPlayerField, onReset } = result.current;
